...
class TejidoApiTest(unittest.TestCase):
    ...
    def test_01c_dj_apolo_and_freestyle_seeds_are_idempotent(self):
        APP.init_db()
        APP.init_db()
        status, publications = self.request("/api/publications")
        self.assertEqual(status, 200)
        ...
        dj_matches = [
            ...
            if item["title"] == "DJ Apolo: sonidos alternativos desde Caucasia"
        ]
        self.assertEqual(len(dj_matches), 1)
        dj = dj_matches[0]
        self.assertEqual(dj["kind"], "TALENTO")
        self.assertEqual(dj["location"], "Caucasia")
        self.assertIsNone(dj["start_date"])
        self.assertIsNone(dj["end_date"])
        self.assertEqual(dj["image"], "/assets/dj-apolo/dj-apolo.jpg")
        self.assertEqual(len(dj["gallery"]), 1)
        self.assertEqual(dj["gallery"][0]["url"], "/assets/dj-apolo/dj-apolo.jpg")
        self.assertIn("Bajo Cauca", dj["content"])
        self.assertIn("techno", dj["content"])

        event_matches = [
            item
            for item in publications
            if item["title"] == "Noche de Freestyle: rimas sin límites"
        ]
        self.assertEqual(len(event_matches), 1)
        event = event_matches[0]
        self.assertEqual(event["kind"], "EVENTO")
        self.assertEqual(event["start_date"], "2026-06-11T20:00")
        self.assertIsNone(event["end_date"])
        self.assertEqual(
            event["location"], "Zona Karaoke, Calle 19, Loma Fresca, Caucasia"
        )
        self.assertEqual(
            event["image"], "/assets/eventos/noche-freestyle-zona-karaoke.jpg"
        )
        self.assertEqual(len(event["gallery"]), 1)
        self.assertIn("colectivo local", event["content"])
        self.assertIn("municipios cercanos", event["content"])

        with APP.db() as connection:
            event_detail = connection.execute(
                "SELECT e.venue FROM events e JOIN publications p ON p.id=e.publication_id WHERE p.title=?",
                ("Noche de Freestyle: rimas sin límites",),
            ).fetchone()
        self.assertIsNotNone(event_detail)
        self.assertEqual(
            event_detail["venue"], "Zona Karaoke, Calle 19, Loma Fresca, Caucasia"
        )
