...
def init_db():
    with db() as c:
        ...
        author=c.execute("SELECT id FROM users WHERE lower(email)='gestor@tejido.co' LIMIT 1").fetchone()
        talent_category=c.execute("SELECT id FROM categories WHERE type='TALENTO' AND active=1 ORDER BY id LIMIT 1").fetchone()
        dj_title="DJ Apolo: sonidos alternativos desde Caucasia"
        dj_cover="/assets/dj-apolo/dj-apolo.jpg"
        dj_summary="DJ residente con trayectoria en el Bajo Cauca y otros lugares del país, con un repertorio que recorre el techno y distintas ramas de la música electrónica."
        dj_content=("DJ Apolo es un DJ residente con trayectoria en el Bajo Cauca y presentaciones en otros lugares del país. "
                    "Su repertorio es variado e incluye techno y distintas ramas de la música electrónica. Su trabajo nace de la pasión por el arte y de su interés por compartir nuevas experiencias sonoras desde Caucasia.")
        dj_gallery=[{
            "url":dj_cover,
            "alt_text":"DJ Apolo mezcla música frente a una controladora en un espacio iluminado con luces de colores.",
            "caption":"DJ Apolo en cabina, compartiendo sonidos alternativos desde Caucasia.",
            "position":0
        }]
        dj=c.execute("SELECT id,image FROM publications WHERE lower(title)=lower(?) AND deleted=0 LIMIT 1",(dj_title,)).fetchone()
        if not dj and author and talent_category:
            created=now()
            cur=c.execute("INSERT INTO publications(author_id,category_id,kind,title,summary,content,image,location,start_date,end_date,link,featured,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(author["id"],talent_category["id"],"TALENTO",dj_title,dj_summary,dj_content,dj_cover,"Caucasia",None,None,None,0,"PUBLISHED",created,created))
            dj_id=cur.lastrowid
            sync_publication_gallery(c,dj_id,dj_gallery)
        elif dj:
            dj_id=dj["id"]
            if talent_category:
                c.execute("UPDATE publications SET category_id=?,kind='TALENTO',summary=?,content=?,location='Caucasia',start_date=NULL,end_date=NULL,status='PUBLISHED',updated_at=? WHERE id=?",(talent_category["id"],dj_summary,dj_content,now(),dj_id))
            if not dj["image"] or str(dj["image"]).startswith("/assets/dj-apolo/"):
                c.execute("UPDATE publications SET image=?,updated_at=? WHERE id=?",(dj_cover,now(),dj_id))
            saved_gallery=c.execute("SELECT url FROM publication_images WHERE publication_id=? ORDER BY position,id",(dj_id,)).fetchall()
            is_managed_gallery=saved_gallery and all(str(row["url"]).startswith("/assets/dj-apolo/") for row in saved_gallery)
            if not saved_gallery or is_managed_gallery: sync_publication_gallery(c,dj_id,dj_gallery)

        event_category=c.execute("SELECT id FROM categories WHERE type='EVENTO' AND active=1 ORDER BY id LIMIT 1").fetchone()
        freestyle_title="Noche de Freestyle: rimas sin límites"
        freestyle_cover="/assets/eventos/noche-freestyle-zona-karaoke.jpg"
        freestyle_location="Zona Karaoke, Calle 19, Loma Fresca, Caucasia"
        freestyle_start="2026-06-11T20:00"
        freestyle_summary="Batallas de freestyle en vivo, artistas locales e invitados de municipios cercanos, premios para los mejores y DJ Apolo como invitado."
        freestyle_content=("Miembros de un colectivo local organizaron una noche de freestyle que reunió a artistas locales e invitados de municipios cercanos. "
                           "El encuentro incluyó batallas en vivo, premios para los mejores y una invitación a compartir rimas, flow y estilo, con DJ Apolo como DJ invitado. "
                           "La actividad se realizó el jueves 11 de junio de 2026 a las 8:00 p. m. en Zona Karaoke, Calle 19, Loma Fresca, Caucasia.")
        freestyle_gallery=[{
            "url":freestyle_cover,
            "alt_text":"Afiche de Noche de Freestyle con un rapero en escena y DJ Apolo como invitado.",
            "caption":"Afiche de Noche de Freestyle, realizada el 11 de junio de 2026 en Zona Karaoke.",
            "position":0
        }]
        freestyle=c.execute("SELECT id,image FROM publications WHERE lower(title)=lower(?) AND deleted=0 LIMIT 1",(freestyle_title,)).fetchone()
        if not freestyle and author and event_category:
            created=now()
            cur=c.execute("INSERT INTO publications(author_id,category_id,kind,title,summary,content,image,location,start_date,end_date,link,featured,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(author["id"],event_category["id"],"EVENTO",freestyle_title,freestyle_summary,freestyle_content,freestyle_cover,freestyle_location,freestyle_start,None,None,0,"PUBLISHED",created,created))
            freestyle_id=cur.lastrowid
            sync_publication_gallery(c,freestyle_id,freestyle_gallery)
            c.execute("INSERT INTO events(publication_id,venue,capacity) VALUES(?,?,?)",(freestyle_id,freestyle_location,None))
        elif freestyle:
            freestyle_id=freestyle["id"]
            if event_category:
                c.execute("UPDATE publications SET category_id=?,kind='EVENTO',summary=?,content=?,location=?,start_date=?,end_date=NULL,status='PUBLISHED',updated_at=? WHERE id=?",(event_category["id"],freestyle_summary,freestyle_content,freestyle_location,freestyle_start,now(),freestyle_id))
            if not freestyle["image"] or str(freestyle["image"]).startswith("/assets/eventos/"):
                c.execute("UPDATE publications SET image=?,updated_at=? WHERE id=?",(freestyle_cover,now(),freestyle_id))
            saved_gallery=c.execute("SELECT url FROM publication_images WHERE publication_id=? ORDER BY position,id",(freestyle_id,)).fetchall()
            is_managed_gallery=saved_gallery and all(str(row["url"]).startswith("/assets/eventos/") for row in saved_gallery)
            if not saved_gallery or is_managed_gallery: sync_publication_gallery(c,freestyle_id,freestyle_gallery)
            event=c.execute("SELECT publication_id FROM events WHERE publication_id=?",(freestyle_id,)).fetchone()
            if event: c.execute("UPDATE events SET venue=? WHERE publication_id=?",(freestyle_location,freestyle_id))
            else: c.execute("INSERT INTO events(publication_id,venue,capacity) VALUES(?,?,?)",(freestyle_id,freestyle_location,None))
