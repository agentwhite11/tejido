#!/usr/bin/env python3
import json, os, re, secrets, hashlib, hmac, mimetypes, webbrowser
...
MYSQL_CONFIG = {
    ...
    "user": os.getenv("MYSQL_USER", "tejido_app"),
    "password": os.getenv("MYSQL_PASSWORD", "Tejido2026!"),
    "database": os.getenv("MYSQL_DATABASE", "tejido_db"),
    "charset": "utf8mb4",
}
...
class CursorWrapper:
    ...
    def _row(self, row):
        return HybridRow(zip(self.cursor.column_names, row)) if row is not None else None
...
class MySQLDB:
    def __init__(self): self.conn=mysql.connector.connect(**MYSQL_CONFIG)
    def execute(self, sql, args=()):
        sql=sql.replace("INSERT OR IGNORE", "INSERT IGNORE").replace("?", "%s")
...
def init_db():
    with db() as c:
        ...
        c.executemany("INSERT IGNORE INTO roles(id,name,description) VALUES(?,?,?)",[(1,"ADMIN","AdministraciÃ³n y moderaciÃ³n total"),(2,"GESTOR","CreaciÃ³n y gestiÃ³n de contenidos"),(3,"CIUDADANO","Consulta, favoritos y reportes")])
        users=[(1,"Valentina Admin","admin@tejido.co","Admin123!"),(2,"Mateo Gestor","gestor@tejido.co","Gestor123!"),(3,"SofÃ­a Ciudadana","ciudadano@tejido.co","Ciudadano123!")]
        for role,name,email,pw in users:
            c.execute("INSERT IGNORE INTO users(role_id,name,email,password_hash,created_at) VALUES(?,?,?,?,?)",(role,name,email,hash_password(pw),now()))
        ...
        c.executemany("INSERT IGNORE INTO categories(name,type,color) VALUES(?,?,?)",cats)
        c.execute("INSERT IGNORE INTO organizations(id,user_id,name,description,contact) VALUES(1,2,'Colectivo RÃ­o Vivo','GestiÃ³n cultural y ambiental desde Caucasia.','hola@riovivo.co')")
        if c.execute("SELECT COUNT(*) FROM publications").fetchone()[0]==0:
            seed=[
            (2,2,'EVENTO','Festival RÃ­o y Sabana 2026','MÃºsica, gastronomÃ­a y emprendimiento junto al rÃ­o Cauca.','Tres dÃ­as para encontrarnos alrededor de los sonidos, sabores e iniciativas que nacen en el Bajo Cauca. HabrÃ¡ tarima local, mercado creativo y recorridos ambientales.','linear-gradient(135deg,#F6C453,#F08A5D)','MalecÃ³n de Caucasia','2026-07-18 15:00:00','2026-07-20 22:00:00','https://example.com/festival',1,'PUBLISHED'),
            (2,1,'HISTORIA','Las manos que tejen memoria','Artesanas de El Pando convierten fibras y relatos en piezas Ãºnicas.','Un grupo de mujeres reÃºne saberes heredados y diseÃ±o contemporÃ¡neo para contar historias del territorio a travÃ©s del tejido.','linear-gradient(135deg,#845EC2,#D65DB1)','El Pando, Caucasia',None,None,None,1,'PUBLISHED'),
            (2,3,'OPORTUNIDAD','Convocatoria Semillas Creativas','Apoyo para jÃ³venes con ideas culturales y comunitarias.','La convocatoria entrega mentorÃ­a y capital semilla a diez proyectos liderados por jÃ³venes de 18 a 28 aÃ±os.','linear-gradient(135deg,#00A896,#89C2D9)','Caucasia',None,'2026-08-15 23:59:00','https://example.com/semillas',1,'PUBLISHED'),
            (2,4,'TALENTO','Samuel Torres: fotografÃ­a del territorio','Un lente joven que encuentra belleza en la vida cotidiana.','Samuel recorre barrios y veredas documentando gestos, oficios y paisajes que suelen pasar desapercibidos.','linear-gradient(135deg,#264653,#2A9D8F)','Caucasia',None,None,None,0,'PUBLISHED'),
            (2,5,'INICIATIVA','Biblioteca al parque','Lecturas, juegos y conversaciÃ³n cada sÃ¡bado.','Una red de voluntarios lleva libros y actividades a parques de distintos barrios para acercar la lectura a niÃ±as, niÃ±os y familias.','linear-gradient(135deg,#3B82F6,#8B5CF6)','Parques de Caucasia','2026-07-04 09:00:00',None,None,0,'PUBLISHED'),
            (2,2,'EVENTO','Mercado Hecho en Caucasia','Emprendimientos locales, mÃºsica y cocina en un solo lugar.','Encuentra marcas locales, productos agrÃ­colas, diseÃ±o y cocina del territorio. Entrada libre.','linear-gradient(135deg,#F59E0B,#EF4444)','Parque de las Banderas','2026-07-11 10:00:00','2026-07-11 19:00:00',None,0,'PUBLISHED')]
            for row in seed:
...
def fix_text(value):
    ...
    for _ in range(2):
        if not any(mark in value for mark in ("Ãƒ","Ã‚","Ã¢")): break
        try:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        user=auth(self)
        with db() as c:
            if path=="/api/health": return self.send_json({"status":"ok","database":"mysql","time":now()})
    ...
    def do_POST(self):
        ...
        with db() as c:
            if path=="/api/auth/login":
                ...
                if not r or not verify_password(pw,r['password_hash']): return self.error(401,"INVALID_CREDENTIALS","Correo o contraseÃ±a incorrectos")
                token=secrets.token_urlsafe(32); exp=(datetime.now(timezone.utc)+timedelta(hours=8)).strftime("%Y-%m-%d %H:%M:%S"); c.execute("INSERT INTO sessions(token,user_id,expires_at) VALUES(?,?,?)",(token,r['id'],exp)); return self.send_json({"token":token,"user":{"id":r['id'],"name":r['name'],"email":r['email'],"role":r['role']}})
            ...
            if path=="/api/publications":
                ...
                if len(title)<5 or len(summary)<10 or len(content)<20: return self.error(400,"VALIDATION","Completa tÃ­tulo, resumen y contenido con informaciÃ³n suficiente")
                fields=(user['id'],int(data.get('category_id',1)),str(data.get('kind','HISTORIA')),title,summary,content,str(data.get('image') or ''),str(data.get('location','Caucasia')),data.get('start_date') or None,data.get('end_date') or None,data.get('link') or None,int(bool(data.get('featured',False))) if user['role']=='ADMIN' else 0,'PUBLISHED' if user['role']=='ADMIN' and data.get('publish') else 'DRAFT',now(),now())
            ...
            if m:
                if not user: return self.error(401,"UNAUTHORIZED","Inicia sesiÃ³n para guardar")
            ...
            if m:
                ...
                pid=int(m.group(1)); c.execute("UPDATE publications SET status='REVIEW',updated_at=? WHERE id=? AND (author_id=? OR ?='ADMIN')",(now(),pid,user['id'],user['role'])); return self.send_json({"message":"Enviado a revisiÃ³n"})
            m=re.fullmatch(r"/api/publications/(\d+)/report",path)
            if m:
                if not user: return self.error(401,"UNAUTHORIZED","Inicia sesiÃ³n para reportar")
    ...
    def do_PUT(self):
        ...
        with db() as c:
            cur=c.execute("UPDATE publications SET category_id=?,kind=?,title=?,summary=?,content=?,image=?,location=?,start_date=?,end_date=?,link=?,status='DRAFT',updated_at=? WHERE id=? AND deleted=0 AND (author_id=? OR ?='ADMIN')",(int(data.get('category_id',1)),str(data.get('kind','HISTORIA')),title,summary,content,str(data.get('image') or ''),str(data.get('location','Caucasia')),data.get('start_date') or None,data.get('end_date') or None,data.get('link') or None,now(),pid,user['id'],user['role']))
    ...
    def do_PATCH(self):
        ...
        if status not in ('PUBLISHED','REJECTED','REVIEW'): return self.error(400,"VALIDATION","Estado no vÃ¡lido")
    ...
    def do_DELETE(self):
        ...
        m=re.fullmatch(r"/api/publications/(\d+)/favorite",path)
        if m:
            if not user: return self.error(401,"UNAUTHORIZED","Inicia sesiÃ³n")
...
def main():
    init_db(); server=ThreadingHTTPServer((HOST,PORT),Handler); url=f"http://{HOST}:{PORT}"; print(f"TEJIDO estÃ¡ disponible en {url}"); print("Presiona Ctrl+C para detener.")
