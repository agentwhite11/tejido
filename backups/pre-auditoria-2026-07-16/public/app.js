...
function formatDate(v){if(!v)return 'Para descubrir';const d=new Date(v);return new Intl.DateTimeFormat('es-CO',{day:'numeric',month:'short',year:'numeric'}).format(d)}
function openModal(id){$('#overlay').classList.remove('hidden');$(id).classList.remove('hidden');document.body.style.overflow='hidden'}
function closeAll(){$$('#overlay,.modal,.drawer').forEach(x=>x.classList.add('hidden'));document.body.style.overflow=''}
...
function renderPublications(){const filtered=state.publications.filter(p=>(state.kind==='TODOS'||p.kind===state.kind)&&(!state.search||[p.title,p.summary,p.location].join(' ').toLowerCase().includes(state.search.toLowerCase())));$('#contentGrid').innerHTML=filtered.map(card).join('');$('#emptyState').classList.toggle('hidden',filtered.length>0);bindCards();renderAgenda();renderOpportunities();renderMap()}
...
function normalizeLocation(value=''){return value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim()}
...
function renderSaved(){const section=$('#guardadas'),nav=$('#savedNav');const logged=!!state.user;section.classList.toggle('hidden',!logged);nav.classList.toggle('hidden',!logged);if(!logged)return;const saved=state.publications.filter(p=>p.kind==='OPORTUNIDAD'&&p.favorite);$('#savedGrid').innerHTML=saved.map(p=>`<article class="saved-card"><button class="remove-saved" data-remove-saved="${p.id}" aria-label="Quitar de guardadas">♥</button><span class="saved-kind">CONVOCATORIA GUARDADA</span><h3>${escapeHTML(p.title)}</h3><p>${escapeHTML(p.summary)}</p><div class="saved-meta"><span>⌖ ${escapeHTML(p.location||'Caucasia')}</span><span>◷ ${formatDate(p.end_date||p.start_date)}</span></div><button class="text-link" data-saved-open="${p.id}">Ver convocatoria →</button></article>`).join('');$('#savedEmpty').classList.toggle('hidden',saved.length>0);$$('[data-saved-open]').forEach(b=>b.onclick=()=>showDetail(+b.dataset.savedOpen));$$('[data-remove-saved]').forEach(b=>b.onclick=()=>toggleFavorite(+b.dataset.removeSaved,b))}
async function showDetail(id){try{const p=await api('/api/publications/'+id);$('#detailContent').innerHTML=`<div class="detail-hero" style="background:${escapeHTML(publicationVisual(p))}"></div><p class="eyebrow">${labels[p.kind]}</p><h2>${escapeHTML(p.title)}</h2><div class="detail-meta"><span>◷ ${formatDate(p.start_date||p.created_at)}</span><span>⌖ ${escapeHTML(p.location||'Caucasia')}</span><span>Por ${escapeHTML(p.author)}</span></div><p><b>${escapeHTML(p.summary)}</b></p><p>${escapeHTML(p.content)}</p><div class="detail-actions"><button class="btn btn-primary" id="detailFav">${p.favorite?'♥ Guardado':'♡ Guardar'}</button><button class="btn btn-soft" id="shareBtn">Compartir</button><button class="btn btn-soft" id="reportBtn">Reportar</button></div>`;openModal('#detailModal');$('#detailFav').onclick=()=>toggleFavorite(id,$('#detailFav'),true);$('#shareBtn').onclick=()=>{navigator.clipboard?.writeText(location.origin+'/#contenido-'+id);toast('Enlace copiado')};$('#reportBtn').onclick=()=>report(id)}catch(e){toast(e.message)}}
async function toggleFavorite(id,button,detail=false){if(!state.user){openModal('#loginModal');return}try{const p=state.publications.find(x=>x.id===id);const isFav=p?.favorite||button.classList.contains('on')||button.textContent.includes('Guardado');const result=await api('/api/publications/'+id+'/favorite',{method:isFav?'DELETE':'POST'});if(p)p.favorite=result.favorite;if(detail)button.textContent=result.favorite?'♥ Guardado':'♡ Guardar';else{button.classList.toggle('on',result.favorite);button.textContent=result.favorite?'♥':'♡'}renderSaved();toast(result.favorite?'Guardado en favoritos':'Retirado de favoritos')}catch(e){toast(e.message)}}
...
async function load(){try{state.categories=await api('/api/categories');state.publications=await api('/api/publications');const me=await api('/api/me');state.user=me.user;renderPublications();renderSession();renderSaved()}catch(e){toast('No pudimos conectar con TEJIDO')};$('#editCategory').innerHTML=state.categories.map(c=>`<option value="${c.id}">${escapeHTML(c.name)}</option>`).join('')}
function renderSession(){const logged=!!state.user;$('#loginBtn').classList.toggle('hidden',logged);$('#userBtn').classList.toggle('hidden',!logged);if(!logged)return;const configs={ADMIN:{label:'ADMINISTRADOR',className:'admin',items:['Moderar publicaciones pendientes','Aprobar o rechazar contenido','Consultar estadísticas y eliminar registros']},GESTOR:{label:'GESTOR DE CONTENIDO',className:'gestor',items:['Crear y editar publicaciones propias','Enviar borradores a revisión','Consultar el estado de cada entrega']},CIUDADANO:{label:'CIUDADANO',className:'ciudadano',items:['Guardar convocatorias favoritas','Consultar contenido publicado','Reportar información incorrecta']}};const cfg=configs[state.user.role];$('#userBtn').textContent=state.user.name.split(' ').map(x=>x[0]).slice(0,2).join('');$('#userSummary').innerHTML=`<span class="role-badge">${cfg.label}</span><h2>Hola, ${escapeHTML(state.user.name.split(' ')[0])}.</h2><p>${escapeHTML(state.user.email)}</p>`;$('#roleCapabilities').className='role-capabilities '+cfg.className;$('#roleCapabilities').innerHTML=`<b>Permisos de este perfil</b><ul>${cfg.items.map(x=>'<li>'+x+'</li>').join('')}</ul>`;$('#createBtn').classList.toggle('hidden',state.user.role==='CIUDADANO');$('#mobileLoginBtn').textContent='Abrir mi perfil'}
async function login(e){e.preventDefault();try{const data=await api('/api/auth/login',{method:'POST',body:JSON.stringify({email:$('#loginEmail').value,password:$('#loginPassword').value})});state.token=data.token;state.user=data.user;localStorage.setItem('tejido_token',data.token);closeAll();toast('Bienvenido a TEJIDO');await load()}catch(e){toast(e.message)}}
...
async function openDrawer(){if(!state.user){openModal('#loginModal');return}openModal('#userDrawer');$('#adminStats').classList.add('hidden');const role=state.user.role;if(role==='CIUDADANO'){const saved=state.publications.filter(p=>p.kind==='OPORTUNIDAD'&&p.favorite);$('#panelTitle').textContent='Mis convocatorias guardadas';$('#panelHelp').textContent='Solo este perfil puede consultar su selección personal y reportar contenidos.';$('#myContent').innerHTML=saved.length?saved.map(p=>`<div class="my-item citizen-favorite"><div><b>${escapeHTML(p.title)}</b><small>${escapeHTML(p.location||'Caucasia')}</small></div><div class="my-actions"><button data-citizen-open="${p.id}">Abrir</button><button data-citizen-remove="${p.id}">Quitar</button></div></div>`).join(''):'<div class="empty-panel">No tienes convocatorias guardadas todavía.</div>';$$('[data-citizen-open]').forEach(b=>b.onclick=()=>showDetail(+b.dataset.citizenOpen));$$('[data-citizen-remove]').forEach(b=>b.onclick=async()=>{await toggleFavorite(+b.dataset.citizenRemove,b);openDrawer()});return}try{const admin=role==='ADMIN';const mine=await api(admin?'/api/publications?status=REVIEW':'/api/publications?mine=1');$('#panelTitle').textContent=admin?'Cola de moderación':'Mis publicaciones';$('#panelHelp').textContent=admin?'Revisa lo enviado por los gestores. Puedes aprobar o rechazar cada publicación.':'Crea borradores, edítalos y envíalos al administrador para revisión.';$('#myContent').innerHTML=mine.length?mine.map(p=>`<div class="my-item"><div class="my-item-top"><b>${escapeHTML(p.title)}</b><span class="status">${statusLabels[p.status]}</span></div>${p.moderation_note?'<small>'+escapeHTML(p.moderation_note)+'</small>':''}<div class="my-actions">${!admin?'<button data-edit="'+p.id+'">Editar</button>':''}${!admin&&p.status==='DRAFT'?'<button data-submit="'+p.id+'">Enviar a revisión</button>':''}${admin&&p.status==='REVIEW'?'<button class="approve" data-approve="'+p.id+'">Aprobar</button><button class="reject" data-reject="'+p.id+'">Rechazar</button>':''}<button data-delete="${p.id}">Eliminar</button></div></div>`).join(''):`<div class="empty-panel">${admin?'No hay publicaciones pendientes de revisión.':'Aún no has creado contenido.'}</div>`;bindMy(mine);if(admin){const s=await api('/api/admin/stats');$('#adminStats').classList.remove('hidden');$('#adminStats').innerHTML=Object.entries({Publicados:s.published,'Por revisar':s.pending,Usuarios:s.users,Reportes:s.reports}).map(([k,v])=>`<div class="stat"><b>${v}</b><span>${k}</span></div>`).join('')}}catch(e){toast(e.message)}}
...
function editItem(p){closeAll();$('#editorTitle').textContent='Editar contenido';$('#editId').value=p.id;$('#editKind').value=p.kind;$('#editCategory').value=p.category_id;$('#editTitle').value=p.title;$('#editSummary').value=p.summary;$('#editContent').value=p.content;$('#editLocation').value=p.location||'';$('#editDate').value=p.start_date?.slice(0,16)||'';$('#editLink').value=p.link||'';openModal('#editorModal')}
function newItem(){closeAll();$('#editorTitle').textContent='Crear contenido';$('#editorForm').reset();$('#editId').value='';$('#editLocation').value='Caucasia';openModal('#editorModal')}
async function saveItem(e){e.preventDefault();const id=$('#editId').value;const payload={kind:$('#editKind').value,category_id:+$('#editCategory').value,title:$('#editTitle').value,summary:$('#editSummary').value,content:$('#editContent').value,location:$('#editLocation').value,start_date:$('#editDate').value||null,link:$('#editLink').value||null};try{await api('/api/publications'+(id?'/'+id:''),{method:id?'PUT':'POST',body:JSON.stringify(payload)});closeAll();toast(id?'Contenido actualizado':'Borrador creado');await load();openDrawer()}catch(e){toast(e.message)}}
...
$$('[data-map-kind]').forEach(b=>b.onclick=()=>{mapKind=b.dataset.mapKind;$$('[data-map-kind]').forEach(x=>x.classList.toggle('active',x===b));renderMap()});$$('[data-close]').forEach(b=>b.onclick=closeAll);$('#overlay').onclick=closeAll;$('#menuBtn').onclick=()=>openModal('#mobileMenu');$('#mobileLoginBtn').onclick=()=>{closeAll();state.user?openDrawer():openModal('#loginModal')};$$('#mobileMenu a').forEach(a=>a.onclick=closeAll);$('#loginBtn').onclick=()=>openModal('#loginModal');$('#userBtn').onclick=openDrawer;$('#loginForm').onsubmit=login;$('#logoutBtn').onclick=logout;$('#createBtn').onclick=newItem;$('#editorForm').onsubmit=saveItem;$('#searchToggle').onclick=()=>{$('#searchInput').focus();location.hash='explorar'};$('#searchInput').oninput=e=>{state.search=e.target.value;renderPublications()};$$('.chip').forEach(b=>b.onclick=()=>{$$('.chip').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.kind=b.dataset.kind;renderPublications()});$$('[data-filter]').forEach(a=>a.onclick=()=>{state.kind=a.dataset.filter;$$('.chip').forEach(x=>x.classList.toggle('active',x.dataset.kind===state.kind));setTimeout(renderPublications,0)});$('#randomBtn').onclick=()=>{const items=state.publications;if(items.length)showDetail(items[Math.floor(Math.random()*items.length)].id)};$$('[data-demo]').forEach(b=>b.onclick=()=>{const accounts={admin:['admin@tejido.co','Admin123!'],gestor:['gestor@tejido.co','Gestor123!'],ciudadano:['ciudadano@tejido.co','Ciudadano123!']};[$('#loginEmail').value,$('#loginPassword').value]=accounts[b.dataset.demo]});$('#newsletterForm').onsubmit=e=>{e.preventDefault();toast('¡Listo! Ya eres parte del hilo.');e.target.reset()};document.addEventListener('keydown',e=>{if(e.key==='Escape')closeAll()});load();
...
function assistantRunAction(key,label=''){
  ...
  assistantPose('lee');
  if(key==='hoy'){
    const events=state.publications.filter(item=>item.kind==='EVENTO');
...
function assistantReply(text){
  ...
  if(q.includes('que es')||q.includes('tejido'))answer=assistantAnswers['que-es'];
    ...
    const found=state.publications.find(p=>[p.title,p.summary,p.location,p.kind].join(' ').toLowerCase().includes(q));
    if(found){
